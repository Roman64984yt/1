...
Время: {time.strftime('%d.%m.%Y %H:%M')}
    ...
    sent = await bot.send_message(ADMIN_CHAT, text, reply_markup=kb, disable_web_page_preview=True)
...
async def main():
    print("Бот запущен → .жалоба | .ж | .админ | .помощь")
